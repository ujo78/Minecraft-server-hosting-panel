...
export default function Players({ socket }) {
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            <div className="sci-fi-panel p-6 opacity-95">
                <div className="corner-accent corner-accent-tl"></div>
                <div className="corner-accent corner-accent-tr corner-accent-purple"></div>

                <h2 className="text-3xl mb-6 text-white flex items-center gap-3 font-['VT323'] uppercase tracking-wide border-b border-[#00f0ff]/20 pb-4">
                    <Users className="w-8 h-8 text-[#00f0ff]" />
                    Online Players <span className="bg-[#00f0ff]/20 text-[#00f0ff] px-3 py-0.5 rounded text-xl ml-2 border border-[#00f0ff]/30">({players.length})</span>
                </h2>

                {players.length === 0 ? (
                    <div className="text-center py-20 text-gray-500 bg-black/20 rounded-lg border border-[#00f0ff]/10 border-dashed">
                        <Users className="w-16 h-16 mx-auto mb-4 opacity-20 text-[#00f0ff]" />
                        <p className="text-xl font-['VT323'] uppercase tracking-widest text-[#00f0ff]/50">No players online</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {players.map((player) => (
                            <div
                                ...
                                className="bg-black/40 border border-[#00f0ff]/10 rounded-lg p-4 hover:bg-white/5 hover:border-[#00f0ff]/50 transition-all cursor-pointer group relative overflow-hidden"
                            >
                                {/* Hover Glow */}
                                ...
                                <div className="flex items-center gap-4 relative z-10">
                                    <div className="relative">
                                        <img
                                            ...
                                            alt={player.username}
                                            className="w-14 h-14 rounded-md border-2 border-black/50 group-hover:border-[#00f0ff] transition-colors shadow-lg"
                                            onError={(e) => {
                                                e.target.src = 'https://mc-heads.net/avatar/steve/64';
                                            }}
                                        />
                                        <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-[#00f0ff] rounded-sm border border-black shadow-[0_0_8px_#00f0ff]" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h3 className="font-['VT323'] text-2xl text-white truncate group-hover:text-[#00f0ff] transition-colors leading-none">
                                            {player.username}
                                        </h3>
            ...
            {selectedPlayer && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4" onClick={() => { setSelectedPlayer(null); setDetailedData(null); }}>
                    <div className="sci-fi-panel p-0 max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl" onClick={(e) => e.stopPropagation()}>
                        <div className="corner-accent corner-accent-tl"></div>
                        <div className="corner-accent corner-accent-tr corner-accent-purple"></div>
                        ...
                        {/* Modal Header */}
                        <div className="flex justify-between items-center p-6 border-b border-[#00f0ff]/20 bg-black/40">
                            <h3 className="text-3xl font-['VT323'] text-white uppercase tracking-wide">Player Details</h3>
                            <button onClick={() => { setSelectedPlayer(null); setDetailedData(null); }} className="text-gray-400 hover:text-white transition-colors hover:rotate-90 hover:text-[#00f0ff]">
                                <X className="w-6 h-6" />
                            </button>
                        </div>
                        ...
                        <div className="overflow-y-auto p-8 custom-scrollbar bg-black/20">
                            {loadingDetails ? (
                                <div className="text-center py-12">
                                    <div className="animate-spin w-12 h-12 border-4 border-[#00f0ff] border-t-transparent rounded-full mx-auto shadow-[0_0_15px_rgba(0,240,255,0.3)]"></div>
                                    <p className="text-[#00f0ff] mt-4 font-['VT323'] text-lg animate-pulse">Scanning Player Data...</p>
                                </div>
                            ) : (
                                <div className="space-y-8">
                                    <div className="flex flex-col items-center relative">
                                        {/* Avatar Halo */}
                                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 bg-[#00f0ff]/20 blur-3xl rounded-full pointer-events-none" />

                                        <img
                                            src={getAvatarUrl(selectedPlayer.username)}
                                            alt={selectedPlayer.username}
                                            className="w-32 h-32 rounded-lg border-4 border-[#00f0ff] shadow-[0_0_20px_rgba(0,240,255,0.3)] relative z-10"
                                            onError={(e) => {
                                                e.target.src = 'https://mc-heads.net/avatar/steve/128';
                                            }}
                                        />
                                        <h4 className="text-4xl font-['VT323'] text-[#00f0ff] mt-4 mb-1 tracking-wide relative z-10">{selectedPlayer.username}</h4>
                                    ...
                                    {detailedData?.playerData && (
                                        <div className="space-y-3">
                                            <h5 className="text-xl font-['VT323'] text-white flex items-center gap-2 border-b border-[#00f0ff]/20 pb-2">
                                                <MapPin className="w-5 h-5 text-[#00f0ff]" /> LIVE DATA
                                            </h5>
                                            <div className="grid grid-cols-2 gap-3">
                                                <div className="bg-black/40 rounded p-3 border border-[#00f0ff]/10">
                                                    <div className="text-[10px] text-gray-500 uppercase tracking-widest">Position</div>
                                                    <div className="text-[#00f0ff] font-mono text-sm mt-1">
                                                ...
                                                <div className="bg-black/40 rounded p-3 border border-[#00f0ff]/10">
                                                    ...
                                                    <div className="text-[#b829dd] font-bold font-['VT323'] text-2xl leading-none mt-1">{detailedData.playerData.xpLevel}</div>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {detailedData?.statistics && (
                                        <div className="space-y-3">
                                            <h5 className="text-xl font-['VT323'] text-white flex items-center gap-2 border-b border-[#00f0ff]/20 pb-2">
                                                <TrendingUp className="w-5 h-5 text-[#b829dd]" /> STATISTICS
                                            </h5>
                                            <div className="grid grid-cols-3 gap-3">
                                                <div className="bg-black/40 rounded p-2 border border-[#00f0ff]/10 text-center">
                                                ...
                                                <div className="bg-black/40 rounded p-2 border border-[#00f0ff]/10 text-center">
                                                    ...
                                                    <div className="text-[#00f0ff] font-['VT323'] text-xl pt-0.5">{formatPlayTime(detailedData.statistics.playTime)}</div>
                                                </div>
                                            </div>
                                        </div>
                                    )}
